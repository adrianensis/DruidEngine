...
#define expected_float_eps(x,exp,eps) tic(); __test__float = (x); toc(); __test__total++; __test__b = (fabsf(__test__float-(exp)) < eps); if (__test__b) { __test__ok++; } __test__output  << (__test__b ? "[ OK ]" : "[ FAIL ]") << "\t" << #x << std::endl << "\t" << "result: " << __PRECISION << __test__float << std::endl << "\t" << "expected: " << __PRECISION << (exp) << std::endl << "\t" << "epsilon: " << __PRECISION << (eps) << std::endl << std::endl
