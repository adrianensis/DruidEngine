...
#define test(theClass) std::chrono::duration<double> __test__totaltime; auto t = std::chrono::high_resolution_clock::now(); std::ofstream __test__outputFile; std::ostringstream __test__output; __test__outputFile.open((std::string(#theClass)+".txt").c_str()); __test__output << std::endl << std::endl << std::endl <<   "===========    " << #theClass << "    ===========" << std::endl << std::endl; theClass __test__aux; bool __test__bool; float __test__float; i64 __test__sint; u64 __test__uint; int __test__ok = 0; int __test__total = 0; bool __test__b
...
#define tock()__test__totaltime+=std::chrono::high_resolution_clock::now()-t
...
#define expected_any(aux,x,exp) tick(); aux = (x); tock(); __test__total++; __test__b = (aux==(exp)); if (__test__b) { __test__ok++; } __test__output  << (__test__b ? "[ OK ]" : "[ FAIL ]") << "\t" << #x << std::endl << "\t" << "result: " << __PRECISION << aux << std::endl << "\t" << "expected: " << __PRECISION << (exp) << std::endl << std::endl
...
#define expected_float_eps(x,exp,eps) tick(); __test__float = (x); tock(); __test__total++; __test__b = (fabsf(__test__float-(exp)) < eps); if (__test__b) { __test__ok++; } __test__output  << (__test__b ? "[ OK ]" : "[ FAIL ]") << "\t" << #x << std::endl << "\t" << "result: " << __PRECISION << __test__float << std::endl << "\t" << "expected: " << __PRECISION << (exp) << std::endl << "\t" << "epsilon: " << __PRECISION << (eps) << std::endl << std::endl
