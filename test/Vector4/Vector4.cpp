...
int main() {
	...
	expected_float(a.dot(a), a.x*a.x + a.y*a.y + a.z*a.z + a.w*a.w);
	expected_float(a.dot(b), a.x*b.x + a.y*b.y + a.z*b.z + a.w*b.w);
	expected_float(a.sqrlen(), a.dot(a));
	expected_float(a.len(), sqrtf(a.sqrlen()));
	expected_float(a.max(),3);
	...
	expected_float_eps(DE::Vector4(a).clamp(0.1f).len(),0.1f,0.1f);
	expected_float_eps(DE::Vector4(a).clamp(0.2f).len(),0.2f,0.1f);
	expected_float_eps(DE::Vector4(a).clamp(0.3f).len(),0.3f,0.1f);
	expected_float_eps(DE::Vector4(a).clamp(0.4f).len(),0.4f,0.1f);
	expected_float_eps(DE::Vector4(a).clamp(0.5f).len(),0.5f,0.1f);
