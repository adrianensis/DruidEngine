...
GLuint RenderContext::createVBO(Array<f32>* data, u32 elementSize, u32 attributeArrayIndex) {
	if(data != nullptr){
		u32 VBO;
		glGenBuffers(1, &VBO);

		glBindBuffer(GL_ARRAY_BUFFER, VBO);
		glBufferData(GL_ARRAY_BUFFER, data->getElementSize() * data->getLength(), data, GL_STATIC_DRAW);
...
GLuint RenderContext::createEBO(Array<u32>* data) {
	if(data != nullptr){
		unsigned int EBO;
		glGenBuffers(1, &EBO);

		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, data->getElementSize() * data->getLength(), data, GL_STATIC_DRAW);
